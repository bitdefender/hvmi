...
static void
fmtstr(char *, size_t *, size_t, const char *, int, int, int);
static void
fmtwstr(char *, size_t *, size_t, const wchar_t *, int, int, int);
static void
fmtint(char *, size_t *, size_t, INTMAX_T, int, int, int, int);
static void
fmtflt(char *, size_t *, size_t, LDOUBLE, int, int, int, int *);
static void
printsep(char *, size_t *, size_t);
static int
getnumsep(int);
static int
getexponent(LDOUBLE);
static int
convert(UINTMAX_T, char *, size_t, int, int);
static UINTMAX_T
cast(LDOUBLE);
static UINTMAX_T
myround(LDOUBLE);
    ...
    while (ch != '\0')
        ...
        case PRINT_S_CONV:
            ...
            case 'S':
                ...
                fmtwstr(str, &len, size, wstrvalue, width,
                        precision, flags);
                break;
            case 'p':
                /*
...
int
_vsnprintf(char *str, size_t size, const char *format, va_list args)
{
    LDOUBLE fvalue;
    INTMAX_T value;
    unsigned char cvalue;
...
static void
fmtstr(char *str, size_t *len, size_t size, const char *value, int width, int precision, int flags)
{
...
/// 2011/09/20 patch
static void
fmtwstr(char *str, size_t *len, size_t size, const wchar_t *value, int width, int precision, int flags)
{
    int padlen, strln; /* Amount to pad. */
    int noprecision = (precision == -1);
    ...
    /* If a precision was specified, don't read the string past it. */
    for (strln = 0; value[strln] != L'\0' && (noprecision || strln < precision); strln++)
        continue;
    ...
    {
        /// !!!
        OUTCHAR(str, *len, size, (char)(*value & 0xFF)); /// !!!
        value++;
    }
    while (padlen < 0)
    {
...
static void
fmtint(char *str, size_t *len, size_t size, INTMAX_T value, int base, int width, int precision, int flags)
{
    UINTMAX_T uvalue;
